...
function EvolutionConfidenceSection() {
  ...
  const titleWords = "Empiece simple hoy y evolucione cuando su negocio lo pida".split(" ");
      ...
      <div className="grid gap-7 lg:grid-cols-[1.08fr_.92fr]">
        <motion.article className="rise group relative min-h-[340px] overflow-hidden rounded-[2rem] bg-[#1D1D1D] p-8 text-white shadow-[0_24px_70px_rgba(29,29,29,.18)] transition duration-500 sm:p-10" initial={{ opacity: 0, y: 28 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.3 }} transition={{ duration: 0.65, ease: "easeOut" }} whileHover={{ y: -6 }}>
          ...
          <div className="relative z-10">
            ...
            <h2 className="text-balance text-4xl font-black tracking-[-0.04em] sm:text-[3.2rem] sm:leading-[0.95]">
              {titleWords.map((word, index) => <motion.span key={`${word}-${index}`} className="inline-block pr-[0.22em]" initial={{ opacity: 0, y: 18, rotateX: 8 }} whileInView={{ opacity: 1, y: 0, rotateX: 0 }} viewport={{ once: true }} transition={{ duration: 0.45, delay: index * 0.025 }}>{word}</motion.span>)}
            </h2>
