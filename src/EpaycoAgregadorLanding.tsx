...
export const designChecks = [
  ...
  { name: "Evolution section upgraded", pass: true },
];
...
function AnimatedTrustSection() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      gsap.utils.toArray<HTMLElement>(".trust-number").forEach((number) => {
        const type = number.dataset.type;
        const end = Number(number.dataset.end || 0);
        const counter = { value: 0 };
        gsap.to(counter, { value: end, duration: type === "day" ? 1.35 : 1.05, ease: "power2.out", scrollTrigger: { trigger: number, start: "top 82%", once: true }, onUpdate: () => { const current = Math.round(counter.value); number.textContent = type === "k" ? `${current}k+` : type === "day" ? `${current}/7` : `${current}`; } });
      });
...
function EvolutionConfidenceSection() {
  ...
  const typeTitle = "Empiece simple hoy y evolucione cuando su negocio lo pida";
  ...
  useEffect(() => {
    ...
    const reduced = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    if (reduced) {
      setTypedChars(typeTitle.length);
      return undefined;
    }
  ...
  useEffect(() => {
    ...
    if (typedChars >= typeTitle.length) return;
    const reduced = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    if (reduced) {
      setTypedChars(typeTitle.length);
      return;
    ...
    const id = window.setInterval(() => {
      setTypedChars((current) => (current >= typeTitle.length ? typeTitle.length : current + 1));
    }, 36);
    return () => window.clearInterval(id);
  }, [typedChars, typingStarted, typeTitle]);
      ...
      <div className="grid gap-7 lg:grid-cols-[1.08fr_.92fr]">
        <motion.article className="rise group relative min-h-[340px] overflow-hidden rounded-[2rem] bg-[#1D1D1D] p-8 text-white shadow-[0_24px_70px_rgba(29,29,29,.18)] transition duration-500 sm:p-10" initial={{ opacity: 0, y: 28 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.3 }} transition={{ duration: 0.65, ease: "easeOut" }} whileHover={{ y: -6 }}>
          ...
          <div className="relative z-10">
            ...
            <h2 className="max-w-4xl text-balance text-4xl font-black tracking-[-0.04em] sm:text-[3.2rem] sm:leading-[0.95]">
              <span className="inline bg-[#ED1C27] px-1.5 py-0.5 text-white box-decoration-clone">{typeTitle.slice(0, typedChars)}</span>
              {typedChars < typeTitle.length ? <span className="ml-1 inline-block h-[1em] w-[2px] align-[-0.1em] bg-white/90 animate-pulse" aria-hidden="true" /> : null}
            </h2>
            <motion.p className="mt-6 max-w-2xl text-lg leading-8 text-white/72 transition duration-300 group-hover:text-white/82" initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.55, delay: 0.18 }}>
            ...
            <motion.div className="mt-8 flex flex-wrap gap-3" initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.26 }}>
            ...
            <div className="mt-8 grid gap-3 sm:grid-cols-2">
              {frictionItems.map((item, index) => {
                ...
                return (
                  <motion.div
                    key={item}
                    className="group/item relative overflow-hidden rounded-[1.1rem] border border-[#1D1D1D]/8 bg-[#F2F2F2] p-4 shadow-[0_8px_18px_rgba(29,29,29,.03)] transition duration-300 hover:bg-white hover:shadow-[0_20px_40px_rgba(29,29,29,.08)]"
                  ...
                  >
                    ...
                    <div className="flex items-start gap-3">
                      <motion.div
                        className={`relative mt-0.5 grid h-8 w-8 shrink-0 place-items-center rounded-full shadow-sm ring-1 transition-colors duration-300 ${isFilled ? "bg-[#009056] text-white ring-[#009056]/35" : "bg-white text-[#009056] ring-[#009056]/12 group-hover/item:bg-[#009056] group-hover/item:text-white"}`}
                        animate={isFilled ? { scale: [1, 1.12, 1], rotate: [0, 8, 0] } : { scale: 1, rotate: 0 }}
                        transition={{ duration: 0.42, ease: "easeOut" }}
                      >
                        <motion.span
                          className={`absolute inset-0 rounded-full transition-colors duration-300 ${isFilled ? "bg-[#009056]/22" : "bg-[#009056]/10"}`}
                          animate={isFilled ? { scale: [1, 1.32, 1], opacity: [0.55, 0.18, 0.55] } : { scale: [1, 1.16, 1] }}
                          transition={{ duration: isFilled ? 1.3 : 2.2, repeat: Infinity, ease: "easeInOut" }}
                        />
                        <Check className="relative z-10 h-4 w-4 stroke-[3]" />
                      </motion.div>
                      <motion.p className="text-sm font-black leading-6 text-[#1D1D1D] transition duration-300 group-hover/item:translate-x-1">
                        {item}
                      </motion.p>
                    </div>
                  </motion.div>
...
function ProductPillar({ product, index, isActive, isMuted, onActivate }: { product: EcosystemProduct; index: number; isActive: boolean; isMuted: boolean; onActivate: () => void }) {
  ...
  return <motion.button type="button" aria-label={`Explorar ${product.name}`} title={`${index + 1}. ${product.name}`} onMouseEnter={onActivate} onFocus={onActivate} onClick={onActivate} initial={{ opacity: 0, y: 100, scaleY: 0.78 }} whileInView={{ opacity: 1, y: 0, scaleY: 1 }} viewport={{ once: true, margin: "-10%" }} transition={{ duration: 0.58, delay: index * 0.018, ease: [0.22, 1, 0.36, 1] }} className={`group absolute bottom-0 w-[7.35%] min-w-[42px] max-w-[68px] origin-bottom rounded-t-full focus:outline-none focus-visible:ring-2 focus-visible:ring-white/70 ${isMuted ? "opacity-35 grayscale" : "opacity-100"}`} style={{ left: `${item.x}%`, height: `${item.h}%`, zIndex: item.z }}><span className={`absolute inset-0 overflow-hidden rounded-t-full border-x border-white/[0.06] bg-[linear-gradient(90deg,#07090D_0%,#151922_18%,#3A3E49_52%,#1B1E27_78%,#07090D_100%)] shadow-[inset_10px_0_18px_rgba(255,255,255,.05),inset_-14px_0_20px_rgba(0,0,0,.52),0_18px_32px_rgba(0,0,0,.22)] transition duration-300 ${isActive ? "brightness-105" : ""}`}><span className="absolute inset-0 bg-[linear-gradient(90deg,transparent,rgba(255,255,255,.08),transparent)] opacity-60" /><span className="absolute inset-0 opacity-22 transition duration-300 group-hover:opacity-30" style={{ background: `linear-gradient(180deg, ${glowColor}18, transparent 52%, rgba(0,0,0,.22))` }} /><span className="absolute inset-x-0 bottom-0 h-14 bg-[linear-gradient(180deg,rgba(0,0,0,0),rgba(0,0,0,.14)_26%,rgba(0,0,0,.42)_68%,rgba(0,0,0,.72)_100%)]" /></span><motion.span animate={{ y: isActive ? -7 : 0, scale: isActive ? 1.03 : 1 }} transition={{ type: "spring", stiffness: 250, damping: 21 }} className="absolute left-1/2 top-0 grid aspect-square w-[clamp(40px,4.7vw,64px)] -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full border border-white/70 bg-white text-[#171717] shadow-[0_10px_30px_rgba(0,0,0,.24)]"><span aria-hidden="true" className={`absolute inset-[-5px] rounded-full blur-sm transition duration-300 ${isActive ? "opacity-28" : "opacity-12 group-hover:opacity-20"}`} style={{ backgroundColor: glowColor }} /><span className="absolute inset-[4px] rounded-full bg-[radial-gradient(circle_at_40%_28%,#fff_0%,#fff_38%,#ececec_100%)]" /><ProductImage product={product} className="relative z-10 h-[66%] w-[66%] object-contain" /></motion.span></motion.button>;
}
...
function PaymentCarousel() {
  const checkoutIndex = ecosystemProducts.findIndex((item) => item.name === "Desarrolladores");
...
function AngledSection({ children, className = "", id }: { children: React.ReactNode; className?: string; id?: string }) { return <section id={id} className={`relative z-10 overflow-hidden ${className}`}><div className="absolute inset-x-0 top-0 h-24 -skew-y-2 bg-inherit origin-top-left" />{children}</section>; }
